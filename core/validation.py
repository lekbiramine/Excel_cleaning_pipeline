...
# class ValidationEngine:
#     """
#     Apply validation rules from rules configuration.
...
#             # --------------------------------------------------
#             # 1️⃣ Apply TYPE conversion first
#             # --------------------------------------------------
...
#             # --------------------------------------------------
#             # 2️⃣ Apply NULL rule AFTER conversion
#             # --------------------------------------------------

#             self._apply_null_rule(df, column, rule)

#         # --------------------------------------------------
#         # Split cleaned vs rejected
#         # --------------------------------------------------
...
#     def _apply_null_rule(self, df: pd.DataFrame, column: str, rule: dict) -> None:
#         """
#         Reject null or empty values if allow_null is False.
...
#     def _apply_numeric_rule(self, df: pd.DataFrame, column: str, rule: dict) -> None:
#         """
#         Convert to numeric and apply min/max constraints.
#         Invalid parsing becomes NaN (caught later by null rule).
#         """

#         df[column] = pd.to_numeric(df[column], errors="coerce")

#         # Apply min rule (only to non-null values)
#         if "min" in rule:
#             mask = df[column].notna() & (df[column] < rule["min"])
#             df.loc[mask, "rejection_reason"] += f"{column}_below_min;"

#         # Apply max rule (only to non-null values)
#         if "max" in rule:
#             mask = df[column].notna() & (df[column] > rule["max"])
#             df.loc[mask, "rejection_reason"] += f"{column}_above_max;"

#     # --------------------------------------------------

#     def _apply_datetime_rule(self, df: pd.DataFrame, column: str, rule: dict) -> None:
#         """
#         Convert to datetime and validate future constraint.
#         Invalid parsing becomes NaT (caught by null rule if required).
#         """

#         df[column] = pd.to_datetime(df[column], errors="coerce")

#         if not rule.get("allow_future", True):
#             mask = df[column].notna() & (df[column] > pd.Timestamp.today())
#             df.loc[mask, "rejection_reason"] += f"{column}_future;"
...
#             ~df[column].astype(str).str.lower().isin(allowed)
#         )
